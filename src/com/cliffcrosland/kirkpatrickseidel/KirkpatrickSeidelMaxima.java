...
public class KirkpatrickSeidelMaxima {
    ...
    //   half at that median. Call the remaining regions L and R.
    // - Look through R and find the point with the highest y coordinate. That is a globally undominated maximum. Add
    //   it to your output.
    // - Delete every point that is dominated by that maximum (i.e. every point that has smaller x coordinate or y
    //   coordinate).
    // - Recurse in region L, and recurse in region R.
    //
    // # PERFORMANCE #
    // Performance can be shown readily to be O(n log n). Also readily shown to be O(n log h) where h is the number of
    ...
    //   line drawn perfectly vertically or horizontally.
    // - Draw k boxes such that the rest of S (the non-maxima) are contained within them. Note that each of the k boxes
    //   is beneath the "stair-case". Call these boxes S1, S2, ... Si, ... Sk. Let's call each possible partition of S
    //   into boxes a "legal box partition".
    // - Let there be an interesting looking function f that takes a legal box partition and produces the following sum
    //   over the boxes (note that |Si| is the number of points in bucket Si):
    //   f(legal_box_partition) = SUM_(i=1)^(k) |Si| * log (n / |Si|)
    // - Let M be the minimum value of f(legal_box_partition) over all possible legal box partition for S.
    // - THEN, the runtime of the algorithm is O(M).
    // - What does this mean? Well, with a crappy legal box drawing, you'd use n boxes, and put each point in a box
    //   all by itself. Then trivially, you get SUM_(i=1)^(n) 1 * log (n / 1) = n log n, which is a trivial worst case
    ...
    //   - Consider the recursive call tree structure. At each level in the recursive call tree, you divide the input
    //     in half, and do at most linear work on each level. We want to see how much each bucket has left at each
    //     level.
    //   - The big idea is that an arbitrary bucket, Si, will have fewer and fewer points left alive in it as we get
    //     deeper in the recursive call tree because its points get lopped off as we find maxima.
    ...
    //         2^j recursive calls, evenly split between each other because we partition by medians in each call. So
    //         each recursive call contains n / 2^j elements. The points still remaining in the box will be between two
    //         maxima, one picked in each recursive call, so the points in the box Si might span at most 2 recursive
    //         calls, so there may be at most 2 * n / 2^j points in Si at level j. We drop the 2 because we're just
    //         interested in the fact that the number of points left in Si at level j will be O(n / 2^j).
    //   - OK cool, now we know that at level j, there will be at most O(n / 2^j) points left in Si. How does this help
    //     us get our crazy sum? Well, as we look at Si's size change over time with each level in the recursion, we
    //     note that in the first few levels, Si might not decrease very much at all, but as soon as n / 2^j becomes
